globals [
  ...
  total-trip-returners
  total-scout-returners
  total-forager-returners
  daily-recruits
  foraging-distances
  ...
  unique-run-id
  persistency-file
  SA-prob-mod                                      ; Multiplier that modifies the probability modulator
  SA-intens-mod                                    ; Multiplier that modifies the intensity modulator
...
turtles-own [
  modulator                        ; the probability and intensity modulator of each individual; random variables drawn from normal distribution
  dance-intensity                  ; the dance intensity of each bee; is a function of sucrose threshold
...
to setup
  ; In our runs of the model, we used a pre-set seed
  __clear-all-and-reset-ticks
  random-seed current-random-seed
  set unique-run-id new-seed
  set SA-prob-mod probability-modulator-modifier
  set SA-intens-mod intensity-modulator-modifier
  ;; SET MODEL PARAMS
...
to setup-turtles
  ...
  ask turtles [
    set id who
    set modulator random-normal-in-bounds 0.255 0.1 0.01 0.5
    set dance-intensity 57 * modulator
...
to go
  ...
  ask returned-scouts with [ moved-this-tick = false ] [
    ...
    if unloading-timer > nest-nectar-handling-time [
      let probability-dance-yes dance-response quality-taught-patch distance-taught-patch modulator ; distance here is in patches, will be in km later
      ...
      if (probability-dance-yes > (random 1000000) / 1000000) [
        ...
        set dance-intensity dance-intens quality-taught-patch distance-taught-patch modulator                                             ; if the bee dances, the intensity is set as dance-intensity
  ...
  ; every visit by an individual to the four patches is recorded in patch specific text files; along with the day the visit was made
  ask foragers with [ moved-this-tick = false ] [ ; there got to be a more elegant way to do this
    set moved-this-tick true
    set flight-timer flight-timer + 1
  ...
  if ticks mod ticks-per-day = 0 [
    ...
    ask idle-foragers [
      let probability-abandon-current-patch probability-abandoning modulator quality-visited-food-patch distance-visited-food-patch
...
to-report dance-response [ pMol pDistance suc-thresh ]
  let result 0
  let deval-mol pMol + 1.02 - distance-factor pDistance
  if deval-mol <= 0.00001 [ set deval-mol 0.00001 ]
  set result 0.956937 / (1 + exp (((SA-prob-mod * suc-thresh) - ln deval-mol) / 0.2624014))
  report result
end
...
to-report dance-intens [ pMol pDistance variable-modulator ]
  let result 0
  let deval-mol pMol + 1.02 - distance-factor pDistance
  if deval-mol <= 0.00001 [ set deval-mol 0.00001 ]
  let val-dance-intens (variable-modulator * SA-intens-mod) + ((1 - SA-intens-mod) * 0.255)
  set result 57 * val-dance-intens * deval-mol
  report floor result
end
...
to-report probability-abandoning [ suc-thresh pMol pDistance ]
  let result 0
  let deval-mol pMol + 1.02 - distance-factor pDistance
  if deval-mol <= 0.00001 [ set deval-mol 0.00001 ]
  set result (1 - (0.956937 / (1 + exp (((SA-prob-mod * suc-thresh) - ln (deval-mol + 0.6687)) / 0.2624014))))
  report result
end
...
944
177
probability-modulator-modifier
2
1
0
Number
...
This model is used to run simulations of the sensitivity analysis for Experiment 1 in the current study. For details of Experiment 1, please see the manuscript as well as the NETLOGO code associated with Experiment 1.

Sensitivity Analysis
In order to test how sensitive our model was to changes in our two individual-level response parameters (response probability and response intensity), we ran a sensitivity analysis based on model 4 in experiment 1. We chose this experiment as the controlled food availability allowed us to check the relative effect of probability and intensity in consistent inter-individual variation in recruitment activity. We chose this model (model 4) as it incorporates individual variation in probability and intensity of recruitment, unlike the other 3 models. We varied the distribution of the probability and intensity modulators to change the extent of inter-individual differences in probability and intensity.
We changed the value of the probability modulator between 0.25 and 2 in steps of 0.25. To change the spread of the individual variation in intensity, we changed the standard deviation of the normal distribution of the intenssity modulator, while keeping the mean constant. The standard deviation was changed from 0.01 to 0.1 in steps of 0.01. We then quantified the consistency in inter-individual variation in the total recruitment activity for each of the 80 combinations (8 levels of variation in probability and 10 levels of variation in intensity).
@#$#@#$#@
default
true
0
...
<experiments>
  <experiment name="Sensitivity-Analysis" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="current-random-seed">
      <value value="377077371"/>
      <value value="377077372"/>
      <value value="377077373"/>
      <value value="377077374"/>
      <value value="377077375"/>
    </enumeratedValueSet>
    <steppedValueSet variable="probability-modulator-modifier" first="0.25" step="0.25" last="2"/>
    <steppedValueSet variable="intensity-modulator-modifier" first="0.1" step="0.1" last="1"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
